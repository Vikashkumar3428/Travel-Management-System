...
def home():
    ...
    def about():
        ...
        rooa.configure(bg='Black')

        g=Frame(height=700,width=1100,bg="White",highlightbackground="Black", highlightthickness=2)
        g.place(x=130,y=20)
        title=Label(rooa, text="About Us...",width=10,height=2,bg="White",font=('Helvetica',40)).place(x=550,y=50)
        k=Label(rooa, text="Wake up to the adventure you've been dreaming about.You're about to embark on a journey. Maybe you're yearing for the peace and sancity of nature ?",width=150,height=2,bg="White",font=('Helvetica',10)).place(x=80,y=250)
        a=Label(rooa, text="Maybe you're on a quest to capture a lots of memory ? Or perhaps you'd rather assimilate the culture and ambience places .",width=150,height=2,bg="White",font=('Helvetica',10)).place(x=80,y=300)
        b=Label(rooa, text="Come on a journey with us and discover the choice of destinations offered to you by VS Travel & Tours.Whether your choice will lie with a superior",width=150,height=2,bg="White",font=('Helvetica',10)).place(x=80,y=350) 
        c=Label(rooa, text="world class hotel, an unrivalled resort, our world-class National Parks, or one of our distinctive bush lodges, the experience begins here, with us.",width=150,height=2,bg="White",font=('Helvetica',10)).place(x=80,y=400)
        d=Label(rooa, text="Currently it is only available to the north India ,but soon will cover south India as well.",width=150,height=2,bg="White",font=('Helvetica',10)).place(x=80,y=450)
        e=d=Label(rooa, text="So come up with your dreams, the stage to be fulfilled by us .",width=150,height=2,bg="White",font=('Helvetica',10)).place(x=80,y=500)
    ...
    def contact():
        ...
        def zzz():
            ...
            messagebox.showinfo("Error","Please fill all the blanks")
        def files():
            ...
            le=len(phone_no)
            message=ee.get()
            if( (first_name=="" and last_name=="" and email=="" and phone_no=="" and message=="") or (first_name==" " and last_name==" " and email==" " and phone_no==" " and message==" ")):
                zzz()
            elif(first_name.isalpha()==False ):
                messagebox.showerror("Warning!","First_name can only contain alphabets")
                home()
            elif(last_name.isalpha()==False ):
                messagebox.showerror("Warning!","Last_name can only contain alphabets")
                home()
            elif(first_name=="" or first_name==" "):
                messagebox.showinfo("Error","Please fill last name")
            elif(last_name=="" or last_name==" "):
                messagebox.showinfo("Error","Please fill last name")
            elif(email=="" or email==" "):
                messagebox.showinfo("Error","Please fill valid email")
            elif "@" not in email:
                messagebox.showinfo("Error","Invalid Email")
            elif ".com" not in email:
                messagebox.showinfo("Error","Invalid Email")
            elif phone_no.startswith("0"):
                messagebox.showerror("Warning ","INVALID NUMBER")    
            elif phone_no.startswith("1"):
                messagebox.showerror("Warning ","INVALID NUMBER")
            elif phone_no.startswith("2"):
                messagebox.showerror("Warning ","INVALID NUMBER")    
            elif phone_no.startswith("3"):
                messagebox.showerror("Warning ","INVALID NUMBER")
            elif phone_no.startswith("4"):
                messagebox.showerror("Warning ","INVALID NUMBER")
            elif phone_no.startswith("5"):
                messagebox.showerror("Warning ","INVALID NUMBER")   
                ...
                for i in phone_no:
                    if i.isalpha()==True:
                        messagebox.showinfo("Error","Please fill valid phone number")
                        break
            elif (phone_no=="" or phone_no==" "):
                messagebox.showinfo("Error","Please fill valid phone number")
            elif (message=="" or message==" "):
                messagebox.showinfo("Error","Please fill the message box")
            else:
                f8=open("Travel.txt","a+")
                f8.write(first_name+" ")
                f8.write(last_name+"\n")
                f8.write(email+"\n")
                f8.write(phone_no+"\n")
        ...
        def abc():
            ...
            ee1=Entry(r,textvariable=aa).place(x=770,y=210)
            ee2=Entry(r,textvariable=bb).place(x=1000,y=210)
            lab3=Label(r,text="ENTER YOUR EMAIL *",font=("helvetica",15),bg="white").place(x=770,y=260)
            ee3=Entry(r,textvariable=cc,width="20").place(x=770,y=300)
            lab4=Label(r,text="ENTER PHONE NUMBER *",font=("helvetica",15),bg="white").place(x=770,y=350)
            ee4=Entry(r,textvariable=dd).place(x=770,y=390)
            lab5=Label(r,text="MESSAGE",font=("helvetica",15),bg="white").place(x=770,y=440)
            ee5=Entry(r,textvariable=ee).place(x=770,y=480)
            bt=Button(r,text="SEND MESSAGE",font=("helvetica",12),bg="green",command=files).place(x=950,y=570)
            
            def homm():
                r.destroy()
                home()
            Button(r,text="Home",command=homm,font=("helvetica",12),bg="green").place(x=1000,y=630)
            r.mainloop()
        abc()
    ...
    def feed():
        ...
        def feedsql():
            ...
            if a=='' or b=='' or c=='' or d=='' or e=='' or f=='' or g=='':
                ...
                messagebox.showerror("WARNING","Invalid Mobile Number")
            elif b==10:
                for i in b:
                    if i.isalpha():
                        messagebox.showerror("WARNING","Invalid Mobile Number")
                ...
                dfed=(a,c,b,d,e,f,g)
                conn.execute(('''INSERT INTO  FEEDBACK('NAME','GENDER','MOBILE','BOOKING_EXPERIENCE','CUSTOMER_SERVICE','CALL_SERVICE','PAYMENTEXPERIENCE')VALUES(?,?,?,?,?,?,?)'''),dfed)
                conn.commit()
                messagebox.showerror("WARNING","FEEDBACK SUBMITTED SUCESSFULLY")
                roof.destroy()
                home()
    ...
    def showbok():
        ...
        def homeb():
            ros.destroy()
            home()
        bu_t=tk.Button(ros,text="home",command=homeb,bg="black",fg="white",width=100,height=1).place(x=300,y=600)
    ...
    def cancel():
        ...
        def dell():
            a=mob_can.get()
            conn.execute('''DELETE FROM TEST2 WHERE MOBILE=?''',(a,))
            conn.commit()
            rooc.destroy()
    ...
    can=tk.Button(root,text="CANCEL YOUR TICKET",bd=0,fg='white',bg='black',font=("lora",15),command=cancel)
    can.place(x=1200,y=100,width=400,height=40)
    fro=StringVar()
    to=StringVar()
    name=StringVar()
    gender=StringVar()
    ...
    def pay():
        ...
        if a=='' or b=='' or c=='' or d=='' or e=='' or f=='' or g=='' or h==''or i=='':
            messagebox.showerror("Warning!","Please Enter the required Fields !")
            home()
        elif(a==b):
            messagebox.showerror("Destination and Arrivial cannot same")
            home()
        
        elif(c.isalpha()==False ):
            messagebox.showerror("Name cannot conatain nubers")
            home()
        elif(le>10 or le<10):
            messagebox.showerror("Warning ","INVALID NUMBER")
        else:
            ...
            def paywith():
                ...
                frr_tk=tk.Frame(roott,bg="lightblue").place(x=100,y=100,width="1200",height="600")
                lab_from=tk.Label(roott,text="BOOKING DETAILS ",font=("lucida calligraphy",30,"bold")).place(x=500,y=150)
                
                lab_from=tk.Label(roott,text="FROM :  ",font=("lucida calligraphy",20,"bold"),bd=0,bg="lightblue").place(x=150,y=250)
                ...
                lab_tot=tk.Label(roott,text="TRAVELLING DATE  : ",font=("lucida calligraphy",20,"bold"),bd=0,bg="lightblue").place(x=150,y=350)
                lab_tod=tk.Label(roott,text=date,font=("lucida calligraphy",20,"bold"),bd=0,bg="lightblue").place(x=450,y=350)
                lab_to3=tk.Label(roott,text="AMOUNT PAID    :",font=("lucida calligraphy",20,"bold"),bd=0,bg="lightblue").place(x=150,y=450)
